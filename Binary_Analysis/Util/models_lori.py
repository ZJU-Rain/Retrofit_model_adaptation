...
def build_or_load_gen_model(args):
    ...
    if args.model_type == 'roberta':
        ...
        print(f"Base model total parameters (excluding LoRA): {base_total:,}")

        # Apply LoRA to the model
        lora_config = LoraConfig(
            r=512,  # Rank of the low-rank adaptation layer, can be adjusted as needed
            lora_alpha=1024,  # Scaling factor for LoRA
            lora_dropout=0.1,  # LoRA dropout
            task_type='SEQ_2_SEQ_LM',  # Task type
            # target_modules = ["q", "v"]
            target_modules=["q", "k", "v", "o", "wi", "wo"]
        )
        model = get_peft_model(model, lora_config)  # Apply LoRA layers to the model
        ...
        for name, param in model.named_parameters():
            num = param.numel()
            total_params += num
            if 'lora_A' in name or 'lora_B' in name:  # LoRA added parameters
                lora_params += num
            if param.requires_grad:  # Trainable parameters
                trainable_params += num

        print(f"Total parameters (including LoRA): {total_params:,}")
        print(f"LoRA parameters: {lora_params:,}")
        print(f"Trainable parameters: {trainable_params:,}")
        print(f"Trainable parameter ratio (including LoRA): {trainable_params / total_params * 100:.4f}%")
        print(f"Trainable parameter ratio (excluding LoRA): {trainable_params / base_total * 100:.4f}%")

        """
        for name, module in model.named_modules():
            # Check if it is a torch.nn.Linear layer
            if isinstance(module, torch.nn.Linear):
                # Get all parameters of the layer
                params = list(module.parameters())
                # Check the requires_grad status of each parameter
                for param in params:
                    print(f"{name: <40} | Requires Grad: {param.requires_grad}")
        """

        # Compile optimized forward graph
        # model = compile(model)
        ...
        print("\n=== After applying LoRA ===")
        model.print_trainable_parameters()  # View trainable parameters after LoRA
        print("\n")
        """
